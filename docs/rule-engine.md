...
Operators: `EQUALS`, `GTE`, `LTE`, `BETWEEN`, `PRESENT`, `ALL_TRUE`, `IN`, `CONTAINS`, `NO_ADMIN`, `NO_PUBLIC_INGRESS`, `S3_ENCRYPTED`. Numeric comparisons accept numeric literals and numeric strings. `BETWEEN` uses inclusive `[minimum, maximum]` bounds; the agent policy pack uses it to reject zero, negative and excessive timeout/step limits. No SpEL, scripts, shell execution, or user-provided Java classes are interpreted. IAM checks examine Allow statements, scalar/list action/resource values and NotAction/NotResource. Ingress checks detect world-open administrative/database ports, IPv4/IPv6, quoted or numeric port ranges and all-protocol access; malformed public port ranges fail closed and ICMP type/code fields are not treated as ports. `S3_ENCRYPTED` requires a nonempty configuration list whose defaults all declare AES256, aws:kms, or aws:kms:dsse. This is an explicit-configuration policy; missing declarations do not prove that S3 service-default encryption is disabled.
